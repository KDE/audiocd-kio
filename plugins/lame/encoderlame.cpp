...
void EncoderLame::loadSettings()
{
    ...
    default:
        ...
        swap = false; // it looks like lame is now clever enough to do the right
                      // thing by default? (#171065)
#else
        swap = false;
#endif
    }
...
long EncoderLame::readInit(long /*size*/)
{
    ...
    *d->currentEncodeProcess << QStringLiteral("-") << d->tempFile->fileName();
    ...
    connect(d->currentEncodeProcess, &KProcess::readyReadStandardOutput, this, &EncoderLame::receivedStdout);
    connect(d->currentEncodeProcess, &KProcess::readyReadStandardError, this, &EncoderLame::receivedStderr);
    // 	connect(d->currentEncodeProcess, &KProcess::bytesWritten,
    //                          this, &EncoderLame::wroteStdin);

    connect(d->currentEncodeProcess, QOverload<int, QProcess::ExitStatus>::of(&KProcess::finished), this, &EncoderLame::processExited);
